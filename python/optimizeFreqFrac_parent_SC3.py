...
# This script should be called as optimizeFreqFrac_parent.py factor fleet nu s0 s1 s2 s3 npopu mprob ntol seed firstchromo mach seed
...
if __name__ == '__main__':  
    ...
    if len(sys.argv)<13:
        print("WARNING!!!! The script has not been called in the form python optimizefreqfrac_parent.py factor fleet nu s0 s1 s2 s3 npopu mprob ntol mach seed")
        sys.exit()
    ...
    # The machine
    mach=int(sys.argv[12])
    np.random.seed(mach)
    print("The machine has been set to %d"%mach)

    # The firstchromo
    if len(sys.argv) > 13:
        firstchromo=sys.argv[13]
    ...
    if (not filecmp.cmp(dirname+'fleetsize.h',dirname+'fleetsize.bck')):
        print('Recompiling')
        comp = subprocess.run(['g++','-O2','simulation.cpp','-o','simulation%d'%mach], cwd=dirname)
    ...
    if os.path.exists(fileroot+'_finished.tmp'): # if the simulation has already been performed, we do nothing
        ...
        if len(sys.argv) == 12:
            [Neval,bestTC,bestTCSD,bestp]=optimization.GAoptimize(INfile,TRfile, RMfile,s,conf,factor,nu,fleet,npopu,mprob,ntol,fileroot, mach)
        
        elif len(sys.argv) == 13:
            [Neval,bestTC,bestTCSD,bestp]=optimization.GAoptimize(INfile,TRfile, RMfile,s,conf,factor,nu,fleet,npopu,mprob,ntol,fileroot, mach, firstchromo)
